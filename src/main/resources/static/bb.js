 ...
 */
/*1497385349,,JIT Construction: v3085030,en_US*/
...
try {
    window.FB || (function (window, fb_fif_window) {
        ...
        (function () {
            ...
            with (this) {
                ...
                __d("JSSDKRuntimeConfig", [], {"locale": "en_US", "rtl": false, "revision": "3085030"});
                __d("JSSDKConfig", [], {
                    "bustCache": true,
                ...
                __d("JSSDKXDConfig", [], {
                    ...
                    "XdBundleUrl": "\/connect\/xd_arbiter\/r\/0F7S7QWJ0Ac.js?version=42",
                ...
                __d('sdk.AppEvents', ['AppUserPropertyAPIBuiltinField', 'Assert', 'ApiClient', 'sdk.Event', 'sdk.Impressions', 'sdk.Model', 'sdk.Runtime', 'FBAppEvents', 'sdk.Auth'], (function a(b, c, d, e, f, g, h, i, j, k, l, m, n, o, p) {
                    var q = {
                        ...
                        }, t = /^[0-9a-zA-Z_][0-9a-zA-Z _-]{0,39}$/, u = t, v = 40, w = 100, x = 100, y = 40, z = 100,
                        aa = ES('Object', 'values', false, h), ba = new m({UserID: ''});

                    function ca(pa, qa, ra, sa) {
                        var ta = {ae: 1, ev: qa, vts: ra, canvas: 1};
                        if (sa) ta.cd = sa;
                        l.impression({api_key: pa, payload: ES('JSON', 'stringify', false, ta)});
                    }

                    function da(pa, qa, ra, sa) {
                        var ta = p.getAuthResponse(), ua = ta && ta.accessToken ? ta.accessToken : null;
                        o.logEvent(pa, qa, sa || {}, ra, ua);
                    }

                    function ea(pa, qa, ra, sa) {
                        fa(qa);
                        var ta = ba.getUserID();
                        if (ta !== '') {
                            sa = sa || {};
                    ...
                    function fa(pa) {
                        i.isTrue(t.test(pa), 'Invalid event name: ' + pa + '. ' + 'It must be between 1 and ' + v + ' characters, ' + 'and must be contain only alphanumerics, _, - or spaces, ' + 'starting with alphanumeric or _.');
                    }

                    function ga(pa, qa, ra, sa) {
                        var ta = {};
                        ta[s.CURRENCY] = ra;
                        ea(pa, r.PURCHASED, qa, babelHelpers['extends']({}, sa, ta));
                    }

                    function ha(pa) {
                        ea(pa, r.ACTIVATED_APP);
                    }
                    ...
                    function ja(pa) {
                        i.isTrue(pa.length !== 0, 'User ID must be set before updateUserProperties can be called.');
                        i.isTrue(pa.length <= w, 'Invalid user ID: ' + pa + '. ' + 'It must be no longer than ' + w + ' characters.');
                    }
                    ...
                    function na(pa) {
                        i.isTrue(ES('Object', 'keys', false, pa).length <= x, 'The total number of user properties cannot exceed ' + x + '.');
                        for (var qa in pa) {
                            i.isTrue(u.test(qa) || ES(aa, 'includes', true, qa), 'Invalid user properties key name: ' + qa + '. ' + 'It must be between 1 and ' + y + ' ' + 'characters, and must be contain only alphanumerics, _, - or spaces, ' + 'starting with alphanumeric or _. ' + 'Or, it must be a pre-defined user property');
                            i.isTrue(pa[qa].toString().length <= z, 'Invalid user properties value: ' + pa[qa] + '. ' + 'It must be no longer than ' + z + ' characters.');
                        }
                    }

                    function oa(pa, qa, ra) {
                        var sa = la();
                        ja(sa);
                        na(qa);
                        var ta = '/' + pa + '/user_properties', ua = {data: [{user_unique_id: sa, custom_data: qa}]};
                        j.graph(ta, 'post', ua, ra);
                    }

                    k.subscribe('init:post', function (pa) {
                        if (n.getClientID()) {
                            if (pa.autoLogAppEvents !== undefined) {
                                i.isBoolean(pa.autoLogAppEvents, 'Type of property autoLogAppEvents must be boolean');
                                n.setAutoLogAppEvents(pa.autoLogAppEvents);
                            }
                            if (n.getAutoLogAppEvents()) ia(n.getClientID());
                        }
                    });
                    f.exports = {
                        activateApp: ha,
                        ...
                        getUserID: la,
                        clearUserID: ma,
                        assertValidUserProperties: na,
                        updateUserProperties: oa
                    };
                }), null);
    ...
    new Image().src = "http:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m=' + encodeURIComponent('{"error":"LOAD", "extra": {"name":"' + e.name + '","line":"' + (e.lineNumber || e.line) + '","script":"' + (e.fileName || e.sourceURL || e.script) + '","stack":"' + (e.stackTrace || e.stack) + '","revision":"3085030","namespace":"FB","message":"' + e.message + '"}}');
}